...
import { CheckCircle2, AlertCircle, Shield, Rocket, ChevronRight, Wand2 } from 'lucide-react';
...
const PubStep: React.FC<PubStepProps> = ({ num, label, info, status }) => {
    ...
    return (
        <div className="flex items-start gap-3 sm:gap-4 py-3">
            <div className={clsx('w-6 h-6 rounded-lg flex items-center justify-center shadow-sm shrink-0 mt-0.5', c.bg)}>
                {c.icon}
            </div>
            <div className="flex-1 min-w-0">
                <p className={clsx('text-[13px] font-bold leading-snug', c.text)}>{label}</p>
...
export function PublicationChecklist({ onPublish, loading }: PublicationChecklistProps) {
    const navigate = useNavigate();
    const { activeProjectId, activeProgrammeId, projects, programmes, user, complianceItems, risks } = useStore();
    ...
    const canPublish     = (activeEntity?.complianceSetupDone || (contextId && (Array.isArray(complianceItems) ? complianceItems : []).some(i => i.projectId === contextId || i.programmeId === contextId))) && 
                           ((activeEntity?.riskSetupDone && activeEntity?.aiRiskDiscoveryDone) || (contextId && (Array.isArray(risks) ? risks : []).filter(r => r.projectId === contextId || r.programmeId === contextId).length >= 3)) &&
                           (!!activeEntity?.deliveryTeamDone);

    const steps: { num: string; label: string; info: string; status: StepStatus }[] = [
        {
            num: '1',
            label: isProg ? '1. Create a programme' : '1. Create a project',
            info: isProg ? 'Initial programme creation and core metadata setup.' : 'Initial project creation and core metadata setup.',
        ...
        },
        {
            ...
            status: (activeEntity?.complianceSetupDone || (contextId && (Array.isArray(complianceItems) ? complianceItems : []).some(i => i.projectId === contextId || i.programmeId === contextId))) 
                ? 'complete' 
                : (activeEntity ? 'active' : 'not-started'),
        },
        {
            num: '3',
            ...
            status: (
                ...
                (Array.isArray(risks) ? risks : []).filter(r => r.projectId === contextId || r.programmeId === contextId).length >= 3
            ) 
                ? 'complete' 
                : ((activeEntity?.complianceSetupDone || (contextId && (Array.isArray(complianceItems) ? complianceItems : []).some(i => i.projectId === contextId || i.programmeId === contextId))) 
                    ? 'active' 
                    : 'not-started'),
        },
        {
        ...
        },
        {
            ...
            status: activeEntity?.isPublished ? 'complete' : (
                canPublish 
                ? 'active' 
                : 'not-started'
            ),
    ...
    const progress       = activeEntity?.isPublished ? 100 : Math.round((completedCount / steps.length) * 100);
            ...
            <div className="bg-slate-50/80 px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 className="text-[13px] font-black text-slate-900 leading-none">Compliance & Risk Setup</h3>
                            <p className="text-[10px] text-slate-500 font-bold mt-1 uppercase tracking-tighter">Project Publication Status</p>
                        </div>
                    </div>
                    <div className={clsx(
                        ...
                        {isComplete ? 'Published' : canPublish ? 'Ready to Publish' : 'Action Required'}
                    </div>
                </div>
                        ...
                        <div key={step.num}>
                            ...
                            {step.num === '2' && (
                                <div className="pl-9 pb-2">
                                    {step.status === 'active' ? (
                                        <button
                                            onClick={() => navigate(`/compliance/setup?type=${activeType}&from=initiation`)}
                                        ...
                                        <button
                                            onClick={() => navigate(`/compliance/dashboard?type=${activeType}&from=initiation`)}
                            ...
                            {step.num === '3' && (
                                <div className="pl-9 pb-2">
                                    {!activeEntity?.riskSetupDone ? (
                                        <button
                                            onClick={() => navigate(`/risk/setup?type=${activeType}&from=initiation`)}
                                            className="text-[10px] font-black text-indigo-600 hover:text-indigo-700 uppercase tracking-widest flex items-center gap-1 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 transition-all active:scale-95"
                                        >
                                        ...
                                        <button
                                            onClick={() => navigate(`/risk/ai?type=${activeType}&from=initiation`)}
                                        ...
                                        <button
                                            onClick={() => navigate(`/risk/dashboard?type=${activeType}&from=initiation`)}
                                            className="text-[10px] font-black text-emerald-600 hover:text-emerald-700 uppercase tracking-widest flex items-center gap-1 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100 transition-all active:scale-95"
                                        >
                                            View Risk Dashboard
                                            <CheckCircle2 className="w-3 h-3" />
            ...
            <div className="px-4 sm:px-6 pb-6 pt-2">
                <button
                    ...
                    onClick={() => {
                        ...
                        if (isComplete) navigate(isClientAdmin ? '/dashboard?type=programme' : '/dashboard?type=project');
                ...
                >
                    {loading ? (
                        ...
                        <><CheckCircle2 className="w-4 h-4" /> VIEW {activeType.toUpperCase()} DASHBOARD</>
                    ) : (
