...
import { isAtLeastClientAdmin, isSuperAdmin, isAtLeastPM, canCreateProject, canCreateProgramme } from '../lib/roles';
...
export function Header() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (user && (isSuperAdmin(email, userRole) || isAtLeastPM(userRole as any) || isAtLeastClientAdmin(userRole as any))) {
        await Promise.all([
          fetchProjects(),
          fetchProgrammes()
        ]);
    ...
    init();
  }, [user, fetchProjects, fetchProgrammes]);
    ...
    <header className="bg-white border-b border-slate-200 h-14 flex items-center px-4 justify-between shrink-0 gap-4">
      ...
      <div className="flex-1 flex items-center gap-2 md:gap-4 min-w-0">
        ...
        <div className="hidden lg:flex items-center gap-2 border-l border-slate-200 pl-4 h-8">
          {canCreateProgramme(userRole as any) && (
            <button
              onClick={() => {
          ...
          {canCreateProject(userRole as any) && (
            <button
              onClick={() => {
                setActiveProject(null);
                setActiveProgramme(null);
