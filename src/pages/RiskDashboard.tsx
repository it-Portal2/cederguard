...
import { Shield, ShieldAlert, BrainCircuit, ArrowRight, Loader2, ShieldCheck, ArrowLeft, CheckCircle2, Zap, AlertTriangle, Target, Binoculars } from 'lucide-react';
import { clsx } from 'clsx';
import { Link, useSearchParams } from 'react-router';
import { stripMarkdown } from '../lib/utils';
import { analyzeStrategicInsights } from '../services/aiService';
import { differenceInMonths } from 'date-fns';
import { isAtLeastClientAdmin, canCreateProject, canCreateProgramme, isSuperAdmin } from '../lib/roles';
import { ShieldAlert as AlertCircle, Wand2, RefreshCcw, Plus, PlusCircle, Layers, Building2, ChevronDown } from 'lucide-react'; 
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          <div className="space-y-1">
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight italic">
              {activeProjectId 
                ? `Risk Dashboard — ${activeProjName || 'Project'}` 
                : activeProgrammeId 
                  ? `Risk Dashboard — ${activeProgName || 'Programme'}`
                  : (userRole === 'client_admin' || userRole === 'admin') ? 'Portfolio Risk Overview' : 'Project Risk Overview'}
            </h1>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">
                {activeProgName || 'Portfolio'} · {new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            {(userRole === 'admin' || userRole === 'client_admin') && (
              <div className="relative group">
                <div className="flex items-center gap-3 px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-xl hover:border-indigo-400 hover:bg-white transition-all cursor-pointer shadow-sm">
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      setActiveProject(''); // Reset project when changing programme
                    ...
                    className="bg-transparent border-none text-[10px] font-black text-indigo-700 uppercase tracking-widest focus:ring-0 appearance-none pr-6 cursor-pointer"
                  >
                    <option value="">All Portfolios</option>
                    {visibleProgrammes.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                  <ChevronDown size={12} className="text-indigo-400 absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none" />
                </div>
              </div>
            )}
            ...
            <div className="relative group">
              <div className="flex items-center gap-3 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl hover:border-indigo-400 hover:bg-white transition-all cursor-pointer shadow-sm">
                ...
                <select 
                  ...
                  className="bg-transparent border-none text-[10px] font-black text-slate-700 uppercase tracking-widest focus:ring-0 appearance-none pr-6 cursor-pointer"
                >
          ...
          <div className="flex items-center gap-3 flex-wrap justify-end">
            ...
            {fromInitiation && (
              <Link 
                ...
                className="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-emerald-200 hover:bg-emerald-700 transition-all active:scale-95 whitespace-nowrap"
              >
                {activeProjectId ? (
                  <>Next: Final Publication <ArrowRight className="w-4 h-4 ml-1" /></>
                ) : (
                  <>Next: Assign Delivery Team <ArrowRight className="w-4 h-4 ml-1" /></>
                )}
              </Link>
            )}
          </div>
      ...
      {strategicInsights && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-3">
            <StrategicInsightCard title="Strategic Outlook & Executive Summary" icon={BrainCircuit} delay="delay-500">
