            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-2">
                <div className="flex items-start gap-4">
                    {fromInitiation && (
                        ...
                        >
                            <ArrowLeft className="w-4 h-4 mr-1" /> Back to Initiation
                        </Link>
                    )}
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                            <FileWarning className="w-6 h-6 text-indigo-600" />
                            {isPM ? 'Shared Issues Portfolio' : 'Programme Issues Log'}
                        </h1>
                        <p className="text-sm text-slate-500 mt-1">Aggregated issues logged across all active projects.</p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <button disabled className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50">
                        <Download className="w-3.5 h-3.5" /> Export
                    </button>
                    {isAtLeastClientAdmin(userRole) && (
                        <button
                            onClick={() => { useStore.getState().setActiveProgramme(null); navigate('/programmes/new'); }}
                            className="flex items-center gap-2 px-6 py-3 bg-white text-indigo-600 border border-indigo-100 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-indigo-50 transition-all shadow-sm active:scale-95"
                        >
