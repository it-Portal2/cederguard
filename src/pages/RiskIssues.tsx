...
import { Trash2, Edit2, Info, Lightbulb, TrendingUp, AlertCircle, ShieldOff, CheckCircle2, ArrowLeft, ArrowRight, Wand2, FileSpreadsheet, Download } from 'lucide-react';
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h1 className="text-3xl font-black text-slate-900 tracking-tight italic">
              Issues Log {activeProjName ? `— ${activeProjName}` : ''}
            </h1>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">
                Active Resolution Tracking · {filtered.length} issues identified
              </p>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-4 md:mt-0">
          {canModify && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              />
              <button
                onClick={() => document.getElementById('issue-file-import')?.click()}
              ...
              >
                <Wand2 className="w-3.5 h-3.5" /> AI Issue Inquiry
              </button>
              <button onClick={() => { setEditingIssue(null); setIsModalOpen(true); }}
          ...
          {selectedIds.length > 0 && canDelete && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-rose-50 text-rose-700 text-sm font-bold rounded-lg hover:bg-rose-100 border border-rose-200 transition-all animate-in fade-in slide-in-from-right-2"
            >
