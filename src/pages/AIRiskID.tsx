...
import { Wand2, AlertTriangle, CheckCircle2, XCircle, ChevronDown, ChevronUp, ShieldAlert, Clock, Info, Eye, RefreshCw, ArrowLeft, Zap, AlertCircle, Plus, Check, X, Edit3 } from 'lucide-react';
...
export function AIRiskID() {
  ...
  const finalize = async () => {
    try {
        ...
        // Use IDs from store or URL for persistence
               ...
               <div className="bg-white rounded-[32px] border border-slate-200 p-8 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
                  ...
                  <div>
                    <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                      <Zap className="w-3 h-3 text-indigo-500" /> Critical Success Factors
                    </h5>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          {r.exists ? (
                            ...
                            <span className="flex items-center gap-1.5 px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-widest rounded-lg border border-indigo-200/50">
                              <Zap className="w-3 h-3 text-indigo-500" /> New Identification
                            </span>
                          )}
                          {isAccepted && (
            ...
            <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto">
               <button 
                 onClick={() => {
                   ...
                   setAcceptedIds([...acceptedIds, ...pendingIds]);
                 }}
                 disabled={pendingRisks.filter(r => !r.exists).length === 0}
                 className="flex-1 sm:flex-none justify-center px-2 sm:px-6 py-2.5 sm:py-3 bg-white border-2 border-slate-200 text-slate-900 text-[10px] sm:text-sm font-black rounded-xl hover:bg-slate-50 transition-all flex items-center gap-1 sm:gap-2 disabled:opacity-50 break-words text-center"
               >
                 <Check className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-500 shrink-0" /> <span className="hidden sm:inline">Accept all</span> remaining ({pendingRisks.filter(r => !r.exists).length})
               </button>
               <button 
                 onClick={finalize}
                 className="flex-1 sm:flex-none justify-center px-2 sm:px-8 py-2.5 sm:py-3 bg-slate-900 text-white text-[10px] sm:text-sm font-black rounded-xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/40 active:scale-95 flex items-center gap-1.5 sm:gap-2.5 break-words text-center leading-tight"
               >
                 <Plus className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400 shrink-0" /> {acceptedIds.length > 0 ? `Add ${acceptedIds.length} risks` : (rejectedIds.length === 0 && safeSuggestedRisks.filter(r => !r.exists).length > 0 ? `Finalise (${safeSuggestedRisks.filter(r => !r.exists).length})` : "Finalise")}
               </button>
            </div>
          </div>
        </div>
