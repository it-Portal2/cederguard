...
import { CheckCircle2, CircleDashed, Clock, FileWarning, TrendingUp, Wand2, AlertCircle, ArrowLeft, ArrowRight, ShieldCheck, Calendar, MessageSquare } from 'lucide-react';
...
export function ComplianceDashboard() {
    const { complianceItems, getActiveItems, getPendingItems, complianceAnalysis, activeProjectId, activeProgrammeId, projects, programmes, user, updateComplianceItem } = useStore();
    ...
    const isPM = !isAtLeastClientAdmin(user?.profile?.role);
    ...
    return (
        <>
        <div className="space-y-6 max-w-7xl mx-auto px-4 md:px-0 pb-12 pb-safe">
            {fromInitiation && (
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-6 border-b border-slate-100 animate-in fade-in slide-in-from-top-4 duration-1000">
                <div className="space-y-1">
                    <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight italic">
                        {activeProjectId 
                            ? `${safeProjects.find(p => p.id === activeProjectId)?.name || 'Project'} Dashboard` 
                            : activeProgrammeId 
                                ? `${safeProgrammes.find(p => p.id === activeProgrammeId)?.name || 'Programme'} Dashboard`
                                : isPM ? 'All Projects Compliance' : 'Portfolio Compliance'
                        }
                    </h1>
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">
                            {isPM || activeProjectId ? 'Project' : 'Programme'} Compliance Posture
                        </p>
                    </div>
                </div>
                <div className="flex gap-2 items-center">
                    {fromInitiation && (
                        <Link 
                            to={!activeDetails.riskSetupDone 
                                ? `/risk/setup?from=initiation${activeProjectId ? `&projectId=${activeProjectId}` : `&programmeId=${activeProgrammeId}`}` 
                                : (activeProgrammeId ? '/programmes/new' : '/initiate')
                            }
                            className="flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-slate-200 hover:bg-indigo-600 transition-all active:scale-95 animate-in fade-in slide-in-from-right-4 duration-700 w-full md:w-auto"
                        >
                            {!activeDetails.riskSetupDone ? (
                                <>Next: Setup Risks <ArrowRight className="w-4 h-4 ml-1" /></>
                            ) : (
                                <>Return to Initiation <ArrowRight className="w-4 h-4 ml-1" /></>
                            )}
                        </Link>
                    )}
                    {complianceAnalysis?.isHRB && (
                        <span className="bg-red-50 border border-red-200 text-red-700 text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm">
                            <FileWarning className="w-3.5 h-3.5" /> HIGHER-RISK BUILDING
                        </span>
                    )}
                    <span className={`border text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm ${complianceAnalysis?.projectRiskLevel === 'High' ? 'bg-red-50 border-red-200 text-red-700' :
                        complianceAnalysis?.projectRiskLevel === 'Medium' ? 'bg-amber-50 border-amber-200 text-amber-700' :
                            'bg-emerald-50 border-emerald-200 text-emerald-700'
                        }`}>
                        RISK: {complianceAnalysis?.projectRiskLevel || 'ASSESSED'}
                    </span>
                </div>
            </div>
            ...
            {activeProgrammeId && milestones.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700 delay-[250ms]">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="max-w-2xl space-y-4">
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/20 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/20 shadow-sm">
                            <Wand2 className="w-3.5 h-3.5 fill-white animate-pulse" />
                            ...
                            return (
                                <div key={item.id} className="bg-red-50 rounded-xl border border-red-100 p-4 shadow-sm flex flex-col justify-between">
                                    ...
                                    <div className="text-[10px] text-red-600 font-black uppercase tracking-widest bg-white/60 p-2 rounded border border-red-50 flex items-center gap-1.5">
                                        <Wand2 className="w-3 h-3 text-indigo-400 shrink-0" /> TRIGGER: {stripMarkdown(item.trigger)}
                                    </div>
                                </div>
                            );
                        })}
            ...
            {complianceAnalysis?.criticalActions?.length > 0 && (
                <div>
                    <h2 className="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-4 border-b border-indigo-200 pb-2 flex items-center gap-2">
                        <Wand2 className="w-4 h-4" /> AI Recommended Actions
                    </h2>
                        ...
                        <ol className="divide-y divide-slate-100">
                            {Array.isArray(complianceAnalysis.criticalActions) && complianceAnalysis.criticalActions.map((action: string, i: number) => (
                                <li key={i} className="flex items-start gap-4 px-6 py-4 hover:bg-indigo-50/30 transition-colors">
                                    <span className="flex-shrink-0 w-7 h-7 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center text-xs font-black">
                                        {i + 1}
                                    </span>
