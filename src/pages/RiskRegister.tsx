...
import { Trash2, Edit2, ShieldOff, AlertCircle, ArrowUpRight, Upload, Download, Wand2, FileSpreadsheet, Plus, AlertTriangle, Flag, RotateCcw, ArrowLeft, ArrowRight } from 'lucide-react';
...
export function RiskRegister() {
  ...
  const canDelete = isAtLeastPM(userRole); // PMs can now delete individual risks
  ...
  return (
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h1 className="text-xl font-black text-slate-900 tracking-tight italic">
              Risk Register {activeProjName ? `— ${activeProjName}` : ''}
            </h1>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">
                {contextLabel} · {filtered.length} risks shown
              </p>
            </div>
          </div>
        </div>
          ...
          {canModify && (
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white text-slate-600 text-[10px] uppercase tracking-wider font-black rounded-xl hover:bg-slate-50 border border-slate-200 transition-all shadow-sm group"
                title="Restart AI analysis flow"
              >
                <RotateCcw className="w-3 h-3 text-slate-400 group-hover:text-indigo-500 transition-colors" /> Restart Analysis
              </button>
              {selectedIds.length > 0 && canDelete && (
                <button
                  onClick={handleBulkDelete}
                  className="flex items-center gap-2 px-4 py-2 bg-rose-50 text-rose-700 text-[10px] uppercase tracking-wider font-black rounded-xl hover:bg-rose-100 border border-rose-200 transition-all shadow-sm animate-in fade-in slide-in-from-right-2"
                >
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-xl hover:bg-emerald-100 border border-emerald-200 transition-all shadow-sm"
          ...
          {canModify && (
            <>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 text-xs font-bold rounded-xl hover:bg-indigo-100 transition-all border border-indigo-200"
              >
                <Wand2 className="w-3.5 h-3.5" /> AI Risk Inquiry
              </button>
              <button onClick={() => { setEditingRisk(null); setIsModalOpen(true); }}
                className="px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-md active:scale-95">
                <Plus className="w-3.5 h-3.5 inline mr-1" /> Add Risk
              </button>
            </>
          )}
        </div>
        ...
        <table className="w-full text-left text-[11px] border-collapse min-w-[1700px]">
          <thead>
            ...
            <tr className="bg-slate-50/80 text-slate-500 uppercase tracking-wider border-b border-slate-200 text-[9px] font-bold sticky top-0 z-10 backdrop-blur-sm">
              ...
              <th className="px-3 py-3 text-center whitespace-nowrap sticky right-0 bg-slate-50 border-l border-slate-100 z-10">Actions</th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-slate-400 whitespace-nowrap font-medium">{ageCalc(r.dateAdded, r.status)}</td>
                   <td className="px-3 py-3 sticky right-0 bg-white z-10 border-l border-slate-100 group-hover:bg-slate-50/80 backdrop-blur-sm">
    ...
    <AIInquiryPopup 
        ...
        context="Risk Register & Analysis"
      />
