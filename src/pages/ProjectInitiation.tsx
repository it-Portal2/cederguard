...
import {
    Save, CheckCircle2, Info, LayoutTemplate,
    UserCircle, Rocket, ChevronRight,
    AlertCircle, Shield, Trash2, Wand2, AlertTriangle, Plus
} from 'lucide-react';
...
export function ProjectInitiation() {
    ...
    const { setActiveProject, setProjects, projects, user, programmes, activeProgrammeId, loadProjectData, updateProject, activeProjectId } = useStore();
    ...
    useEffect(() => {
        const isNewRoute = pathname.includes('/projects/new') || pathname === '/initiate';
        
        if (urlProjectId) {
            setActiveProject(urlProjectId);
            loadProjectData(urlProjectId);
            
            const p = (Array.isArray(projects) ? projects : []).find(proj => proj.id === urlProjectId);
            if (p) {
                setFormData({
                    name: p.name || '',
    ...
    const loadDummy = () => setFormData(prev => ({
        ...prev,
        name: 'Demo Project — Tower A Retrofit',
        type: 'Retrofit / Decarbonisation',
        loc: 'London',
        description: 'Full external wall insulation and window replacement for Tower A to meet EPC C standard.',
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        if (!requiredDone) { setError('Please fill all required fields.'); return; }
        setLoading(true);
        setError('');
        try {
            if (activeProjectId) {
                ...
                if (res?.id) {
                    ...
                    await loadProjectData(res.id);
                    const fetchFn = ['admin', 'pro', 'enterprise', 'client_admin'].includes(userRole)
                        ? api.clientGetProjects : api.getProjects;
                    const listRes = await fetchFn();
                    const projectsList = (listRes as any)?.projects;
                    if (Array.isArray(projectsList)) {
                        const unique = Array.from(new Map(projectsList.map((item: any) => [item.id, item])).values());
                    ...
                    setTimeout(() => {
                        setSuccess(false);
                        navigate(`/compliance/setup?type=project&id=${res.id}&from=initiation`);
                    }, 1400);
                } else {
    ...
    const inputCls = 'w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors';
    const labelCls = 'block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1';
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    ...
                    >
                        ...
                        <div>
                            <h3 className="font-bold text-slate-900 leading-tight">Compliance Profiler</h3>
                            <p className="text-[11px] text-slate-500 mt-0.5">Interactive guide to determine required compliance domains.</p>
                        </div>
                    </Link>
                    <Link 
                    ...
                    >
                        ...
                        <div>
                            <h3 className="font-bold text-slate-900 leading-tight">Risk Identifier</h3>
                            <p className="text-[11px] text-slate-500 mt-0.5">Industry-standard risk library and mitigation reference.</p>
                        </div>
                    </Link>
                </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div className="space-y-2">
                            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-50 border border-indigo-100/50 text-indigo-600">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest">Setup Phase</span>
                            </div>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <button
                                ...
                                className={clsx(
                                    ...
                                    requiredDone && !success
                                        ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-100 hover:shadow-indigo-200'
                                        : 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200 shadow-none'
                                )}
                            >
                                {loading ? (
                                    <><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Working…</>
                                ) : success ? (
                                    <><CheckCircle2 className="w-4 h-4" /> Saved!</>
                                ) : activeProjectId ? (
                                    <><Save className="w-4 h-4 text-indigo-200" /> Save Details</>
                                ) : (
                                    <><Rocket className="w-4 h-4 text-indigo-200" /> Create Project</>
                                )}
                            </button>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-8 order-2 lg:order-1 space-y-8">
                        {/* Success banner */}
                        {success && (
                            <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl px-5 py-4 text-emerald-700 text-sm font-bold shadow-sm animate-in fade-in slide-in-from-top-4">
                        ...
                        {/* Error banner */}
                        {error && (
                            <div className="flex items-center gap-3 bg-rose-50 border border-rose-200 rounded-2xl px-5 py-4 text-rose-700 text-sm font-bold shadow-sm">
                        ...
                        <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-3xl p-6 md:p-10 shadow-xl shadow-slate-200/50 space-y-10">
                            
                            {/* Continuing Drafts Overlay-style section */}
                            {(Array.isArray(projects) ? projects : []).filter(p => !p.isPublished).length > 0 && (
                                <div className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100/50 mb-8 overflow-hidden relative group">
                                    ...
                                    <div className="relative z-10">
                                        <div className="flex items-center justify-between mb-4">
                                            <div>
                                                <p className="text-[11px] font-black text-indigo-600 uppercase tracking-widest leading-none">In-Progress Drafts</p>
                                                <p className="text-[10px] text-slate-500 font-bold mt-1">Pick up where you left off or start fresh.</p>
                                            </div>
                                            {activeProjectId && (
                                                <button 
                                                    type="button"
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3">
                                            <select 
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (id) {
                                                        ...
                                                        loadProjectData(id);
                                                        const safeProjects = Array.isArray(projects) ? projects : [];
                                                        const p = safeProjects.find(proj => proj.id === id);
                                                        if (p) {
                                                            setFormData(prev => ({
                                                                ...prev,
                                                                name: p.name,
                                            ...
                                            >
                                                ...
                                                {(Array.isArray(projects) ? projects : []).filter(p => !p.isPublished).map(p => (
                                                    <option key={p.id} value={p.id}>{stripMarkdown(p.name)} ({calculateProjectProgress(p).percentage}% Complete)</option>
                                                ))}
                                            </select>
                                            {activeProjectId && !(Array.isArray(projects) ? projects : []).find(p => p.id === activeProjectId)?.isPublished && (
                                                <button
                                                    type="button"
                                                    onClick={() => {
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <h2 className="text-sm font-black text-slate-900 uppercase tracking-widest">Metadata & Identity</h2>
                                </div>
                                
                                <div className="space-y-5">
                                    <div>
                                        <label className={labelCls}>Project Name <span className="text-rose-500 ml-0.5">*</span></label>
                                        <input type="text" className={inputCls} placeholder="e.g. Bermondsey Estate Transformation"
                                            value={formData.name} onChange={e => set('name', e.target.value)} required />
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div>
                                            <label className={labelCls}>Project Type <span className="text-rose-500 ml-0.5">*</span></label>
                                            <select className={inputCls} value={formData.type} onChange={e => set('type', e.target.value)} required>
                                        ...
                                        <div>
                                            <label className={labelCls}>Location / Ward <span className="text-rose-500 ml-0.5">*</span></label>
                                            <input type="text" className={inputCls} placeholder="e.g. London"
                                                value={formData.loc} onChange={e => set('loc', e.target.value)} required />
                                        </div>

                                        <div>
                                            <label className={labelCls}>Associated Programme</label>
                                            <select className={inputCls} value={formData.programmeId} onChange={e => set('programmeId', e.target.value)}>
                                        ...
                                        <div>
                                            ...
                                            <select className={inputCls} value={formData.riba} onChange={e => set('riba', e.target.value)}>
                                                <option value="">— Select RIBA Stage —</option>
                                                {RIBA_STAGES.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
                                            </select>
                                        </div>

                                        <div>
                                            <label className={labelCls}>Cost Centre Code</label>
                                            <input type="text" className={inputCls} placeholder="e.g. CC-1234"
                                                value={formData.costCentreCode} onChange={e => set('costCentreCode', e.target.value)} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <h2 className="text-sm font-black text-slate-900 uppercase tracking-widest">Scope & Assets</h2>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div className="md:col-span-2">
                                        <label className={labelCls}>Scope of Works</label>
                                        <textarea rows={3} className={inputCls} placeholder="Main project objectives and deliverables…"
                                            value={formData.description} onChange={e => set('description', e.target.value)} />
                                    </div>

                                    <div>
                                        <label className={labelCls}>Funding Streams</label>
                                        <select multiple className={clsx(inputCls, 'min-h-[120px] scrollbar-hide')}
                                            value={formData.fundingStreams}
                                            onChange={e => set('fundingStreams', Array.from(e.target.selectedOptions, o => (o as HTMLOptionElement).value))}>
                                            <option value="Grant">Government Grant</option>
                                            <option value="RTB receipts">Right to Buy Receipts</option>
                                            <option value="S106 income">Section 106 Income</option>
                                            <option value="Sales income">Private Sales Income</option>
                                            <option value="Private finance">Private Finance / Loans</option>
                                            <option value="Other">Other Internal Funds</option>
                                        </select>
                                        <p className="text-[10px] text-slate-400 font-bold mt-2 uppercase tracking-tight italic">Hold Ctrl / Cmd to multi-pick</p>
                                    </div>

                                    <div className="space-y-4">
                                        <div>
                                            <label className={labelCls}>Storeys</label>
                                            <select className={inputCls} value={formData.numberOfStoreys} onChange={e => set('numberOfStoreys', e.target.value)}>
                                        ...
                                        <div>
                                            ...
                                            <input type="number" className={inputCls} placeholder="e.g. 150"
                                                value={formData.numberOfUnits} onChange={e => set('numberOfUnits', e.target.value)} />
                                        </div>

                                        <div>
                                            <label className={labelCls}>Type of Units</label>
                                            <select className={inputCls} value={formData.typeOfUnits} onChange={e => set('typeOfUnits', e.target.value)}>
                                                <option value="">— Select Type —</option>
                                                <option>Flat</option>
                                                <option>House</option>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" className={inputCls} placeholder="e.g. 2B4P, 1B2P"
                                                value={formData.bedroomsPerProperty} onChange={e => set('bedroomsPerProperty', e.target.value)} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <DeliveryTeamCRUD 
                                    ...
                                    onUpdate={async (data) => {
                                        ...
                                        if (urlProjectId || activeProjectId) {
                                            await updateProject(urlProjectId || activeProjectId || '', data);
                                        }
                                    }}
                                />
                            </div>
                            ...
                            <div className="space-y-6 pb-2">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <h2 className="text-sm font-black text-slate-900 uppercase tracking-widest">Key Timeline</h2>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input type="date" className={inputCls}
                                            value={formData.startOnSite} onChange={e => set('startOnSite', e.target.value)} />
                                    </div>
                                    <div>
                                        <label className={labelCls}>Expected PC Date</label>
                                        <input type="date" className={inputCls}
                                            value={formData.targetPC} onChange={e => set('targetPC', e.target.value)} />
                                    </div>
                                </div>
                            </div>
                        </form>
