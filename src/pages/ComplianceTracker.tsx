      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-6 border-b border-slate-200 animate-in fade-in slide-in-from-top-4 duration-1000">
        <div className="space-y-1">
          <h1 className="text-xl font-black text-slate-900 tracking-tight italic">
            {isPM || activeProjectId 
              ? `${projects.find(p => p.id === activeProjectId)?.name || 'Project'} Compliance Tracker`
              : `${programmes.find(p => p.id === activeProgrammeId)?.name || 'Programme'} Compliance Tracker`
            }
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 md:justify-end">
          {fromInitiation && (
            <Link 
              to="/dashboard"
              className="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-slate-900 text-white rounded-2xl font-black text-[9px] sm:text-[10px] uppercase tracking-wider sm:tracking-[0.2em] shadow-xl shadow-slate-200 hover:bg-indigo-600 transition-all active:scale-95"
            >
              Final Step: View Project Dashboard <ArrowRight className="w-4 h-4 ml-1" />
            </Link>
          )}
          {!isComplianceLocked ? (
            <button 
              onClick={() => setComplianceLocked(true)}
              className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-slate-100 text-slate-900 border border-slate-200 rounded-xl font-bold text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95 shadow-sm"
            >
              <Lock className="w-3.5 h-3.5" /> Finalise
            </button>
          ) : (
            <button 
              onClick={() => setComplianceLocked(false)}
              className="flex items-center gap-2 px-3 sm:px-4 py-2 border rounded-xl font-bold text-[9px] sm:text-[10px] uppercase tracking-widest shadow-sm transition-all active:scale-95 bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-100 group"
              title="Click to unlock tracker"
            >
              ...
              <span className="hidden group-hover:block">Unlock</span>
            </button>
          )}
          <button 
            onClick={() => {
            ...
            className="flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 active:scale-95"
          >
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <aside className="w-full lg:w-72 shrink-0 space-y-6 lg:sticky lg:top-24">
              ...
              <div className="bg-slate-900 rounded-2xl p-6 text-white overflow-hidden relative group">
                ...
                <Zap className="absolute -right-4 -bottom-4 w-24 h-24 text-white/5 rotate-12 group-hover:rotate-45 transition-transform duration-700" />
              </div>
            </aside>
