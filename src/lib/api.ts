...
async function callApi(action: string, body: any = {}, timeout: number = 60000) {
    ...
    try {
        ...
        if (err.name === 'AbortError') {
            throw new ApiError('Request timed out after ' + (timeout / 1000) + ' seconds. The AI might be taking too long or the connection is unstable.', 408);
        }
        throw err;
    }
}
...
export const api = {
    ...
    updateProject: (id: string, data: any) => callApi('updateProject', { id, data }),
    updateProgramme: (id: string, data: any) => callApi('updateProgramme', { id, data }),
    ...
    inviteProjectManager: (pmEmail: string, pmName: string, pmRole: string) => callApi('inviteProjectManager', { pmEmail, pmName, pmRole }),
    clientGetPMs: () => callApi('clientGetPMs'),
    clientGetTeam: () => callApi('clientGetTeam'),
    clientRemoveUser: (targetUid: string) => callApi('clientRemoveUser', { targetUid }),
    clientUpdateUserRole: (targetUid: string, role: string) => callApi('clientUpdateUserRole', { targetUid, role }),
    ...
    analyzeCompliance: (prompt: string, config: any) => callApi('analyzeCompliance', { prompt, config }),
    analyzeRisks: (prompt: string, config: any) => callApi('analyzeRisks', { prompt, config }),
    analyzeControls: (prompt: string, config: any) => callApi('analyzeControls', { prompt, config }),
    chatWithAI: (messages: any[]) => callApi('chatWithAI', { messages }),

    saveData: (collection: string, data: any, projectId?: string | null) => callApi('saveData', { collection, data, projectId }),
    getData: (collection: string, projectId?: string | null) => callApi('getData', { collection, projectId }),
    getSystemMappings: () => callApi('getSystemMappings'),

    getEvidence: (projectId: string) => callApi('getEvidence', { projectId }),
    addEvidence: (projectId: string, document: any) => callApi('addEvidence', { projectId, document }),
    ...
    deleteUserAccount: (targetUid?: string) => callApi('deleteUserAccount', { targetUid }),

    adminStats: () => callApi('adminStats'),
    adminGetUsers: () => callApi('adminGetUsers'),
    adminUpdateUser: (targetUid: string, updates: any) => callApi('adminUpdateUser', { targetUid, updates }),
    ...
    adminUpdatePricingConfig: (config: any) => callApi('adminUpdatePricingConfig', { config }),

    adminCreateInvoice: (invoice: any) => callApi('adminCreateInvoice', { invoice }),
    ...
    adminTransferProgramme: (id: string, targetUser: any) => callApi('adminTransferProgramme', { id, targetUser }),
    adminTransferProject: (id: string, targetUser: any) => callApi('adminTransferProject', { id, targetUser }),
    ...
    upsertComplianceLibraryItem: (item: any) => callApi('upsertComplianceLibraryItem', { item }),
    deleteComplianceLibraryItem: (id: string) => callApi('deleteComplianceLibraryItem', { id }),
    getComplianceDomains: () => callApi('getComplianceDomains'),
    upsertComplianceDomain: (name: string) => callApi('upsertComplianceDomain', { name }),

    // User Preferences
    savePreference: (key: string, value: any) => callApi('savePreference', { key, value }),
